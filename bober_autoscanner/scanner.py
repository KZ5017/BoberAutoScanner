...
def write_report_section(report_handle, section_name, output):
    report_handle.write("\n")
    report_handle.write("=" * 80 + "\n")
    report_handle.write(f"[ {section_name} ]\n")
    report_handle.write("=" * 80 + "\n\n")
    report_handle.write(output)
    report_handle.write("\n")
...
def move_tree_contents(src_base, dst_base, label):
    ...
    for root, dirs, files in os.walk(src_base):
        ...
        for file in files:
            ...
            try:
                shutil.move(src, dst)
                print(f"\033[1m\033[32m[LOOT]\033[0m {label}: {dst}")
            except Exception:
                pass
...
def extract_ports(rustscan_output_file, target_ip):
    ...
    # Look for the exact IP -> [portlist] pattern.
...
def parse_bober_exec_output(output):
    ...
    for line in lines:
        ...
        # Service name is expected at the beginning of the line.
        parts = line.split()
...
def parse_smb_rid_brute(output):
    ...
    for line in output.splitlines():
        ...
        # Only keep user-type SIDs.
        if "SidTypeUser" not in line:
            continue

        # Example: 500: OVERWATCH\Administrator (SidTypeUser)
...
def handle_ldap_loot(name, output):
    ...
    if name == "pre2k" and re.search(r"Saved to .*\.nxc/modules/pre2k/.*", output):
        move_tree_contents(
            os.path.expanduser("~/.nxc/modules/pre2k"),
            "pre2k",
            "PRE2K output moved to current directory"
        )
...
def execute_ldap_block(target_ip, username, password, dc_host, auth_type):
    ...
    with open(output_filename, "w") as report:
        ...
        for entry in LDAP_COMMANDS:
            ...
            # Handle LDAP loot generated by specific modules.
...
def collect_nxc_logs():
    move_tree_contents(
        os.path.expanduser("~/.nxc/logs"),
        "nxc_logs",
        "NXC logs moved to current directory"
    )
...
def discover_web_domains(nmap_full_output, target_ip):
    ...
    for i in range(1, len(port_blocks), 2):
        ...
        if not candidates:
            # We confirmed this is HTTP and not a Windows internal HTTP service.
            ...
            # Determine the scheme more reliably.

            scheme = "http"  # default conservative fallback

            # Explicit HTTPS indicators in the service field.
            ...
            # Check the full block for TLS certificate evidence.
...
def run_web_crawler(target, target_ip, auto_mode):
    ...
    # -----------------------------------
    # Ask whether the crawler should run.
    # -----------------------------------
    if not ask_user(
...
def expand_web_targets(validated_targets, wordlist_for_subdomain, target_ip):
    ...
    while web_queue:
        ...
        target_id = build_target_identifier(target)
        ...
        # Only enumerate vhosts on the root domain target.
...
def scan_web_targets(final_targets, target_ip, crawler_hosts_updated, wordlist_for_endpoints, auto_mode):
    ...
    for target in final_targets:
        ...
        # Run the crawler only when the hosts file was updated.
...
def parse_ffuf_vhost_results(json_file, parent_target):
    ...
    for entry in results:
        ...
        new_host = f"{fuzz_value}.{parent_target['host']}".lower()

        if new_host in seen_hosts:
            continue

        seen_hosts.add(new_host)
        ...
        new_target = {
            ...
            "is_domain": True,
            "supports_vhost_enum": False,  # Stop recursive vhost enumeration here.
            "vhost_fuzzed": True
        }
...
def fetch_url(target, target_ip, path="/", head=False):
    ...
    # Remove the empty method flag when `head` is False.
...
def print_sub_section_title(title, color_code="36"):
    ...
    print("-" * (len(title) + 6))  # Visual separator.
